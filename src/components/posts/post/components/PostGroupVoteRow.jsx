...
import { FaEdit, FaFlag, FaTrash, FaCheck, FaTimes } from "react-icons/fa";
...
const PostMetaRow = ({ post }) => {
    ...
    // const isAdmin = profile?.role === "admin"; // Assuming profile has a role field to check for admin
    ...
    const handleAcceptPost = async () => {
        try {
            const response = await postService.acceptPost(post._id);
            Utils.dispatchNotification(
                "Post approved successfully",
                "success",
                dispatch
            );
            // Optionally, update the post state in Redux or refetch the post
            dispatch(updatePostItem({ ...post, status: "approved" }));
        } catch (error) {
            Utils.dispatchNotification(
                error.response?.data?.message || "Error approving post",
                "error",
                dispatch
            );
        }
    ...
    const handleDeclinePost = async () => {
        try {
            const response = await postService.declinePost(post._id);
            Utils.dispatchNotification(
            ...
            // Optionally, update the post state in Redux or refetch the post
            dispatch(updatePostItem({ ...post, status: "declined" }));
        } catch (error) {
            Utils.dispatchNotification(
                error.response?.data?.message || "Error declining post",
                "error",
                dispatch
            );
        }
                    ...
                    {post?.status === "pending" && (
                        <>
                            <button
                                ...
                                className="flex items-center gap-1 px-2 h-7 rounded-full hover:bg-green-50 focus:outline-none focus:ring-2 focus:ring-green-400 transition"
                            ...
                            <button
                                aria-label="Decline post"
                                className="flex items-center gap-1 px-2 h-7 rounded-full hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-red-400 transition"
                            ...
                            >
                                <div className="py-2">
                                    ...
                                    {isPostOwner && (
                                        <>
                                            <button
                                                className="flex items-center gap-3 w-full px-4 py-3 text-sm text-left text-gray-700 hover:bg-primary hover:text-white transition-all duration-200 group focus:outline-none focus:bg-primary focus:text-white"
