...
const PostContent = ({ post, editor, viewContainerRef, lightboxOpen, setLightboxOpen, currentImageSrc, handleContentClick, backgroundImageColor, setImageUrl, setShowImageModal, showImageModal }) => (
    ...
    {!post.htmlPost && (
      <>
        {post?.post && post?.bgColor === "#ffffff" && (
          <p className="post" data-testid="user-post">{post?.post}</p>
        )}
        {post?.post && post?.bgColor !== "#ffffff" && (
          <div data-testid="user-post-with-bg" className="user-post-with-bg">
            {post?.post}
          </div>
        )}
        {post?.imgId && !post?.gifUrl && post.bgColor === "#ffffff" && (
          <div
            ...
            style={{ height: "auto", backgroundColor: `${backgroundImageColor}` }}
            onClick={() => {
