    ...
    static messageData({
        ...
        if (!isGroupChat) {
            chatConversationId = find(
                chatMessages,
                (chat) =>
                    chat.receiverId === searchParamsId ||
                    chat.senderId === searchParamsId
            );
        }
    ...
    static socketIOMessageReceived(
        ...
        socketService?.socket?.on("message received", (data) => {
            if (
                ...
                if (
                    data.conversationId === ChatUtils.conversationId ||
                    data.groupId === ChatUtils.conversationId
                ) {
        ...
        socketService?.socket?.on("message read", (data) => {
            if (
                ...
                const findMessageIndex = findIndex(
                    ChatUtils.privateChatMessages,
                    ["_id", data._id]
                );
                if (findMessageIndex > -1) {
                    ChatUtils.privateChatMessages.splice(
                        findMessageIndex,
                        1,
                        data
                    );
