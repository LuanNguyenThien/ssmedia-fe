...
export class NotificationUtils {
  ...
  static async socketIOMessageNotification(
    ...
    await socketService?.socket?.on('chat list', (data) => {
      ...
      if (data?.receiverUsername === profile?.username || data?.senderUsername === profile?.username || data?.isGroupChat) {
        let chatListData = null;
        if(data.isGroupChat) {
          chatListData = find(chatList, (chat) => chat.groupId === data.groupId);
        }
