...
const DiscoverSpaces = ({ exploreGroups, onJoinGroup, joinedGroups }) => {
  ...
  useEffect(() => {
    if (Array.isArray(joinedGroups)) {
      const activeMembersByGroup = {};

      joinedGroups.forEach((group) => {
        const active =
          group.members?.filter((member) => member.status === "active") || [];
        activeMembersByGroup[group.id] = active;
      });

      console.log("Active Members by Group:", activeMembersByGroup);
      setActiveMembers(activeMembersByGroup);
    }
  }, [joinedGroups]);
  
  // useEffect(() => {
    
  //     console.log("Joined Groups Members:", joinedGroups);
  //     console.log("exploreGroups", exploreGroups);
  //     const filteredMembers = joinedGroups.members.filter(
  ...
  // }, );
// useEffect(() => {console.log("exploreGroups", exploreGroups);})
  
  const handleLoadMore = () => {
    console.log("ád", exploreGroups);
    console.log("Joined Groups Members:", joinedGroups);
      console.log("exploreGroups", exploreGroups);
    ...
    <div className="w-full h-full overflow-y-auto bg-gray-50 rounded-md">
      <div className="p-6">
        ...
        <div className="mb-6 text-primary-black">
          ...
          <p className="text-gray-500 text-lg">Spaces you might like</p>
        </div>
        ...
        {exploreGroups.length > 0 && (
          <div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 mb-8">
              {exploreGroups.slice(0, displayCount).map((space) => (
                <SpaceCard
                  key={space.id}
                  space={space}
            ...
            {/* Load More Button */}
            {exploreGroups.length > displayCount && (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between mt-1">
                        <span className="text-xs text-gray-500">
                          {(
                            activeMembers[group.id]?.length || 0
                          ).toLocaleString()}{" "}
                          members
                        </span>
                      </div>
