...
export default function MemberRequests({ group, canViewContent, isGroupAdmin }) {
        ...
        const fetchPendingRequests = async () => {
            if (canViewContent && isGroupAdmin && group?._id) {
                try {
                    setLoading(true);
                    console.log(
                        "Fetching pending requests for group:",
                        group._id
                    );
                    const response = await groupService.getpendinguser(
                        group._id
                    );
    ...
    if (pendingRequests.length === 0) {
        return (
            <div className="bg-white rounded-lg shadow-sm p-6 text-center">
                ...
                <p className="text-gray-600">
                    All join requests have been processed. New requests will
                    appear here.
                </p>
            </div>
        );
                            ...
                            <div className="flex-grow min-w-0">
                                <div className="flex items-start justify-between mb-2">
                                    ...
                                    <div className="flex items-center gap-2 text-xs text-gray-500">
                                        <FaClock size={12} />
                                        <span>{timeAgo.transform(request.joinedAt)}</span>
                                    </div>
                                </div>
