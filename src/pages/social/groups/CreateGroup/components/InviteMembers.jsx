...
const InviteMembers = ({ invitedMembers, onInviteMember, onRemoveInvite }) => {
    ...
    const toggleMemberSelect = (user) => {
        const isAlreadySelected = invitedMembers.some(
            (member) => member.userId === user._id
        );
        ...
        <section>
                ...
                <div className="selected-members mb-6">
                    ...
                    <div className="flex flex-wrap gap-4">
                        {invitedMembers.map((member) => (
                            <div
                                key={member.userId}
                                className="selected-member relative"
                            >
                                <Avatar
            ...
            <div className="member-search-results relative">
                {isLoadingMember ? (
                    ...
                    <div className="space-y-2 max-h-48 overflow-y-auto">
                        {searchResults.map((user) => (
                            <MemberCard
                                key={user._id}
                                user={user}
                                isSelected={invitedMembers.some(
                                    (member) => member.userId === user._id
                                )}
                    ...
                    <div className="no-results text-center py-4 text-gray-500">
                        No users found
                    </div>
                ) : null}
            </div>
            ...
            {invitedMembers.length === 0 &&
                ...
                !isLoadingMember && (
                    <div className="text-center py-8 text-gray-500">
                        ...
                        <p className="text-sm">
                            Search for users to invite them
                        </p>
                    </div>
                )}
        </section>
