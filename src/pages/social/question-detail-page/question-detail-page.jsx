...
const QuestionDetail = () => {
  ...
  useEffect(() => {
    const fetchQuestion = async () => {
      try {
        ...
        if (response.data && response.data.post) {
          ...
          Utils.dispatchNotification('Question not found', 'error', dispatch);
        }
      } catch (error) {
        Utils.dispatchNotification(error.response?.data?.message || 'Error fetching question', 'error', dispatch);
      } finally {
        setLoading(false);
      }
    };
    ...
    const loadAnswers = async (page = 1) => {
      try {
        ...
        const response = await answerService.getAnswersForQuestion(questionId, page);
  ...
  return (
    <>
      <div className="saves col-span-full sm:rounded-t-3xl size-full flex justify-center items-center pt-4" data-testid="question-detail">
        <div className="saves-content" style={{ height: '100%', width: '100%' }}>
          <div className="saves-post w-full" style={{ height: '85vh' }}>
            <div className="posts-container w-full overflow-y-auto" data-testid="question-answers">
              
              {/* Question Section */}
              {!loading && question && (
                <div key={question._id} data-testid="question-item" className="mb-4">
                  {(!Utils.checkIfUserIsBlocked(profile?.blockedBy, question?.userId) || question?.userId === profile?._id) && (
                    <>
                      {PostUtils.checkPrivacy(question, profile, profile?.following) && (
                        <>
                          <Post post={question} showIcons={false} />
                        </>
                      )}
                    </>
                  )}
              ...
              {!loading && question && answers.length > 0 && (
                <div className="space-y-4">
                  {answers.map((answer, index) => (
                    <div key={answer._id || index} data-testid="answer-item">
                      {(!Utils.checkIfUserIsBlocked(profile?.blockedBy, answer?.userId) || answer?.userId === profile?._id) && (
                        <>
                          {PostUtils.checkPrivacy(answer, profile, profile?.following) && (
                            <>
                              <Post post={answer} showIcons={true} />
                            </>
                          )}
                        </>
                      )}
                  ...
                  {hasMoreAnswers && (
                    <div className="text-center py-4">
                      ...
                      >
                        {answersLoading ? 'Loading...' : 'Load More Answers'}
                      </button>
                    </div>
                  )}
                </div>
              ...
              {!loading && question && answers.length === 0 && (
                <div className="bg-white rounded-lg shadow-sm">
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-400 mb-6">Be the first to answer this question!</p>
                    <button
                      onClick={handleAddAnswer}
